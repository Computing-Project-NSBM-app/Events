...
const useSchema =yup.object().shape({
  caption: yup.string().required(' Caption Should be Below 1000 Characters').max(1000),
  title:yup.string().required(),
  imageurl:yup.string().required('A URL is Required').url(),
  dates: yup.number().positive().max(31).min(0).required(),
...
export default function AddEvent() {
  ...
  const submit=(vnb=>{
    ...
    const data = post('/api/event', {caption: "caption", clubtitle:"clubtitle", detail:"detail", imageurl:"imageurl"}).then(res => {
  ...
  return (
    ...
    >
      ...
      initialValues={{caption:'',title:'', imageurl:'', dates:'', months:''}}
      onSubmit={values => {
        submit(values)
      }}
      ...
      >
        {({handleSubmit, handleChange, handleBlur, values, errors, isValid})=>(
          ...
          <TextInput
              ...
              placeholderTextColor='gray'
              onChangeText={handleChange('dates')}
              ...
              onChange={(e)=>setthumbnail(e.nativeEvent.text)}
              style={{ color: 'white', fontSize: 15,marginTop:5, marginHorizontal:20, paddingVertical:20 }}
              placeholder='Enter the Event Month '
              placeholderTextColor='gray'
